...
def generate_reports(df):
    ...
    df.to_csv('T1_extracted_data.csv', index=False)
    df.to_excel('T1_extracted_data.xlsx', index=False)
    
    # PDF report
    c = canvas.Canvas('T1_extracted_data.pdf', pagesize=letter)
    text = df.to_string(index=False)
    c.drawString(100, 750, text)
    c.save()
    
    print('Reports have been successfully generated: T1_extracted_data.csv, T1extracted_data.xlsx, and T1_extracted_data.pdf')
...
def main():
    ...
    features_input = input("Enter only 3 features to extract (comma-separated, choose from the available features): ").strip().split(',')
